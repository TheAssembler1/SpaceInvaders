...
#define TEST_BYTE_VALUE 0xFA
#define TEST_SHORT_VALUE 0xFFAA
...
void test_read_write_mem(){
    ...
    for(int i = 0; i < 10; i++){
        if(TEST_BYTE_VALUE == read_byte_mem(i))
            ...
            log_info("FAILED::byte r/w | Expected Value:%x | Actual Value:%x", TEST_BYTE_VALUE, read_byte_mem(i));
    }
    ...
    for(int i = 0; i < 10; i += 2){
        if(TEST_SHORT_VALUE == read_short_mem(i))
            ...
            log_info("FAILED::short r/w | Expected Value:%x | Actual Value:%x", TEST_SHORT_VALUE, read_short_mem(i));
    }
}