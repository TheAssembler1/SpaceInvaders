...
#define __LOG__(format, loglevel, ...)                                                             \
    printf("%s %-5s [%s] [%s:%d] " format "\n", get_formated_time(), loglevel, __func__,           \
           __SHORT_FILE__, __LINE__, ##__VA_ARGS__)
#define __LOG_NONEWL__(format, loglevel, ...)                                                      \
    printf("%s %-5s [%s] [%s:%d] " format, get_formated_time(), loglevel, __func__,                \
           __SHORT_FILE__, __LINE__, ##__VA_ARGS__)
#define __LOG_BARE__(format, ...) printf(format "\n", ##__VA_ARGS__)
#define __LOG_BARE_NONEWL_(format, ...) printf(format, ##__VA_ARGS__)
