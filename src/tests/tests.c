...
void test_read_write_mem() {
    ...
    for (int i = 0; i < 10; i++) {
        if (TEST_BYTE_VALUE == read_byte_mem(i))
            log_info("SUCCESS::byte r/w | Expected Value:%x | Actual Value:%x", TEST_BYTE_VALUE,
                     read_byte_mem(i));
        else
            log_error("FAILED::byte r/w | Expected Value:%x | Actual Value:%x", TEST_BYTE_VALUE,
                      read_byte_mem(i));
    }
    ...
    for (int i = 0; i < 10; i += 2) {
        if (TEST_SHORT_VALUE == read_short_mem(i))
            log_info("SUCCESS::short r/w | Expected Value:%x | Actual Value:%x", TEST_SHORT_VALUE,
                     read_short_mem(i));
        else
            log_error("FAILED::short r/w | Expected Value:%x | Actual Value:%x", TEST_SHORT_VALUE,
                      read_short_mem(i));
    }
}
